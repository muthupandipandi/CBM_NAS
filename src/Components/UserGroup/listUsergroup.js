...
export default class listGroupType extends Component {
  constructor(props) {
    super(props);
    this.state = {
      activePage:1,
  ...
  handleViewUserGroup = (index) => {
		this.setState({view:true, editIndex:index})
  }
  componentDidMount(){
    this.props.UserGroupLoad()
  ...
  mappingreturnData = (startOffset, startCount, Per_Page ) => {
    ...
    // let userData=[{groupName:'NasMamber',descriptions:"Group Description",groupType:'Inbound'},
    // {groupName:'NasMamber',descriptions:"Group Description",groupType:'Outbound'},
    // {groupName:'NasMamber',descriptions:"Group Description",groupType:'Outbound'},]
    ...
    if(!_.isEmpty(userData)){
        ...
        if(index >= startOffset & startCount < Per_Page){
          ...
          return(
              <tr key={index}>
								...
								<td className='text-left'><span className='colorblack '>{val.usergroupName}</span></td>
                                <td className='text-left'><span className='colorblack '>{val.usergroupDesc}</span></td>
                                <td className='text-left'><span className='colorblack '>{val.usergroupType}</span></td>
                ...
                placement="top"
                >
                <span><i className="fas fa-eye m-r-20" style={{color:'blue'}}  onClick={()=>this.handleViewUserGroup(index)}></i></span>
      ...
      return (<tr><td colSpan="4"><center>No Record Found</center></td></tr>)
    }
  }
  handleEdit = (index) => {
  ...
  autoHide = () => {
    setTimeout(() => {
      this.props.UserGroupEditErrorClose()
      }, 5000);
  }
  handleAdd = () => {
...
handleAddClose = () => {
  ...
  render() {
    const {loggedinData, showMessage, message, isPending,isOpen} = this.props;
    const {editIndex,userData,Per_Page,activePage,add,editing,view} = this.state;
    let totalPages ;
    if(!_.isEmpty(userData)){
    ...
    return (
      <div className={isOpen ? "app_Content container-fluid" : "app_ContentSmall container-fluid"}>        
        ...
        <ShowModalLogin message={message} falseShowModalPopUp={this.props.UserGroupEditErrorClose}/> : null}
					{editing === true ? 
						<EditUSerGroup  edit={add ? [] : userData[editIndex]} action={this.props} userData={this.state.userData} loggedinData={loggedinData} add={add} closeModal={this.handleEditingClose} />
                        : null}
          
          {view === true ?  
          <ViewUserGroup edit={add ? [] : userData[editIndex]} action={this.props} 
            add={add} closeModal={this.handleviewCampaignClose} /> : null}
        	{add === true ? 
            <AddUserGroup edit={add ? [] : userData[editIndex]} action={this.props} userData={this.state.userData}  loggedinData={loggedinData} add={add} 
        ...
        <div>
            ...
            <div className='alignRight clearing marginBottom'>
                ...
                <Button className="inlineBlock" onClick={this.handleAdd}> <span><i className="fas fa-plus-square m-r-20" ></i> </span> Add User Group</Button>
            ...
            <Table responsive striped bordered hover>
                <thead>
                  <tr>   
                    ...
                    <th className='text-left'>GroupType</th>
